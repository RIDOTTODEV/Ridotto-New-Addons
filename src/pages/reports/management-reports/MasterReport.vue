...
const getMasteryReportCashDeskFigures = computed(() => {
  return (
    masteryReport.value?.cashdeskFigures?.map((item) => {
      return item.cashdeskTransactionType
    }) || []
  )
})
...
const onSubmitFilter = async () => {
  masteryReport.value = await reportStore.getMasteryReport(dateFilterValue.value)
}

const expansionItemHeaders = ref({
  showSlotFiguresHeader: true,
})
...
watch(getMasteryReportCashDeskFigures, (val) => {
  if (val) {
    expansionItemHeaders.value.showCashdeskFiguresHeader = true
    val.forEach((item) => {
      expansionItemHeaders.value[`showCashdeskFigures${item}Header`] = true
    })
  }
})
</script>
  ...
  <q-page class="q-pa-md app-cart grey-card">
    ...
    <div class="row grey-card q-mt-md">
      ...
      <q-card class="no-box-shadow col-12">
        <div class="text-h6 q-mt-sm text-blue-8 text-bold" v-if="masteryReport?.slotFigures">
          Slot Figures
        </div>
        <Information
        ...
        <q-expansion-item
          ...
          v-if="masteryReport.slotFigures !== null"
          @show="
            () =>
              expansionItemsHeaderAction(
                'showSlotFiguresHeader',
                !expansionItemHeaders.showSlotFiguresHeader,
              )
          "
          @hide="
            () =>
        ...
        >
            ...
            <div class="row col-12">
              <div class="text-subtitle1 text-bold">
                ...
                <span
                  ...
                  v-if="expansionItemHeaders.showSlotFiguresHeader"
                  >Totals: {{ priceAbsFormatted(masteryReport.slotFigures?.totalResult) }}</span
                >
              </div>
            </div>
          </template>
          <div class="row">
            ...
            <div class="col-12 flex justify-end" v-if="false">
              <div class="text-subtitle2 text-bold text-indigo text-right q-mr-md">
                Gaming Win: <span class="q-ml-md">(14,399.92)</span>
              </div>
              <div class="text-subtitle2 text-bold text-indigo text-right">
                Gaming Win: <span class="q-ml-md">(14,399.92)</span>
              </div>
            </div>
          </div>
        </q-expansion-item>
        <div class="text-h6 q-mt-sm text-blue-8 text-bold" v-if="masteryReport?.cashdeskFigures">
          Cashdesk Figures
        </div>
        <Information
          class="full-width q-pa-sm"
        ...
        <q-expansion-item
          ...
          @show="
            () =>
              expansionItemsHeaderAction(
                `showCashdeskFigures${figure.cashdeskTransactionType}Header`,
                !expansionItemHeaders[`showCashdeskFigures${figure.cashdeskTransactionType}Header`],
              )
          "
          @hide="
            () =>
        ...
        >
          <template v-slot:header>
            <div class="row col-12">
              <div class="text-subtitle1 text-bold">
                {{ $t(figure?.cashdeskTransactionType) }}
                <span
                  class="q-ml-md text-deep-orange-10"
                  v-if="
                    expansionItemHeaders[
                      `showCashdeskFigures${figure.cashdeskTransactionType}Header`
                    ]
                  "
                >
            ...
            <div class="row col-12">
              <div class="text-subtitle1 text-bold">
                ...
                <span class="q-ml-md text-deep-orange-10"
                  >Totals: {{ priceAbsFormatted(masteryReport.cashFigures?.totalMovement) }}</span
                >
              </div>
            </div>
          </template>
            ...
            <q-markup-table class="no-box-shadow col-12" dense>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="text-center">
                    {{ priceAbsFormatted(item?.selectCurrencyMovementValue) }}
                  </td>
                </tr>
                <tr>
                  <td class="text-center text-bold borderTop"></td>
